...
plot(par(mar=c(0,4.1,4.1,2.1)), hist(elevation, breaks=100, axes=FALSE, main = "Everglades Elevations", xlab="", ylab = ""))
...
plot(par(mar=c(5.1, 4.1, 0, 2.1), mgp=c(3,0.5,0.0)), boxplot(elevation, horizontal = TRUE, axes=FALSE), axis(1), mtext("elevation (m)", side=1, line=2.5, font=2))
par(old_par)
if (saveAsPng) { dev.off() }
if (saveAsPng) {png(file="Everglades_elevationByHabitat.png", width = 500, height = 500)}
par(mar=c(2.1, 9.1, 2.1, 0.1), las=1)
boxplot(elevation ~ habitat, horizontal=TRUE)
title("Distribution of Elevation by Habitat Type")
plot(par(mar=c(2.1, 9.1, 2.1, 0.1), las=1),boxplot(elevation ~ habitat, horizontal=TRUE), title("Distribution of Elevation by Habitat Type"))
par(old_par)
if (saveAsPng) {dev.off()}
fit <- aov(elevation~habitat,data=everglades_df)
summary(fit)
if (saveAsPng) {png(file="Everglades_elevationByQuad2.png", width = 500, height = 750)}
par(mar=c(2.1,9.1,2.1,5.1), las=1)
boxplot(elevation ~ quad, horizontal=TRUE, outline=FALSE, cex.axis=0.4)
title("Distribution of Elevation by Quad Name")
plot(par(mar=c(2.1,9.1,2.1,5.1), las=1), boxplot(elevation ~ quad, horizontal=TRUE, outline=FALSE, cex.axis=0.4), title(main = "Distribution of Elevation by Quad Name"))
par(old_par)
if (saveAsPng) { dev.off() }